layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
  ...
  table.render({
    elem: '#dataTable'
    ,url:'/data/readLabel/list/'
    ,cellMinWidth: 80
    ,cols: [[
      {field:'id',   title: 'ID'},
  {field:'labelCode',   title: '标签代码'},{field:'labelName',   title: '标签名字'},{field:'type',   title: '标签类型'},
...
$('#searchForm .layui-btn').on('click', function(){
 ...
 //监听工具条
   table.on('tool(table)', function(obj){
     ...
     if(layEvent === 'detail'){
       location.href="/page/readLabel/info?id="+data.id;
     } else if(layEvent === 'del'){
       layer.confirm('确定删除？', function(index){
            layer.close(index);
            $.ajax({
                url:'/data/readLabel/delete',
                data:{id:data.id},
                method:"post",
                dataType:"json",
        ...
        location.href="/page/readLabel/edit?id="+data.id;
     }
   });
});